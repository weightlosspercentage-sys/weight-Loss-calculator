...
UNIFIED_HEADER = """<header class="static-header" style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-bottom: 1px solid #e2e8f0; padding: 0.75rem 1.5rem; position: sticky; top: 0; z-index: 50; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.05); font-family: sans-serif;">
        <style>
...
def process_file(filepath):
    try:
        ...
        # Find <header class="static-header"...</header>
        header_match = re.search(r'(<header class="static-header"[\s\S]*?</header>)', content)
        if header_match:
            old_header = header_match.group(1)
            content = content.replace(old_header, UNIFIED_HEADER)

        # Remove duplicate google translate script if placed before header
        content = re.sub(r'<style>\s*\.goog-te-gadget-simple[\s\S]*?</script>\s*<script type="text/javascript" src="//translate\.google\.com/translate_a/element\.js\?cb=googleTranslateElementInit"></script>', '', content)

        # Replace footer
        footer_match = re.search(r'(<footer[\s\S]*?</footer>)', content)
        if footer_match:
            old_footer = footer_match.group(1)
            content = content.replace(old_footer, UNIFIED_FOOTER)
...
def main():
    ...
    print(f"==================================================")
    print(f"[+] Successfully unified {count} HTML files!")
    print(f"==================================================")
