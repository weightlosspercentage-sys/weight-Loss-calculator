"""
Upgrade Static Headers — weightlosspercentage.com
...
if __name__ == '__main__':
    print("Scanning workspace to upgrade static headers...")
    html_files = collect_html_files(ROOT)
    print(f"Found {len(html_files)} HTML files.")
    
    fixed = 0
    for file_path in html_files:
        # Skip node_modules or build folders
        if "node_modules" in file_path or "dist_old" in file_path or "dist_test" in file_path:
            continue
        if fix_file(file_path):
            fixed += 1
            if fixed % 50 == 0 or fixed < 10:
                print(f"  [+] Upgraded header: {os.path.relpath(file_path, ROOT)}")
                
    print(f"\nCompleted! Upgraded headers in {fixed} files.")
