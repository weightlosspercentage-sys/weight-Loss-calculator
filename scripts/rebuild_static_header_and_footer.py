...
FULL_HEADER = """<header class="static-header" style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-bottom: 1px solid #e2e8f0; padding: 0.75rem 1.5rem; position: sticky; top: 0; z-index: 50; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.05); font-family: sans-serif;">
        <style>
...
FULL_FOOTER = """<footer class="static-footer" style="background: #0f172a; color: #94a3b8; padding: 3rem 1.5rem 2rem; margin-top: 4rem; font-family: sans-serif;">
...
def rebuild_file(filepath):
    try:
        ...
        # Find starting index of <header class="static-header"
        start_hdr = content.find('<header class="static-header"')
        if start_hdr != -1:
            end_hdr = content.find('</header>', start_hdr)
            if end_hdr != -1:
                old_hdr_block = content[start_hdr:end_hdr + 9]
                content = content.replace(old_hdr_block, FULL_HEADER)

        # Replace footer
        start_ftr = content.find('<footer')
        if start_ftr != -1:
            end_ftr = content.find('</footer>', start_ftr)
            if end_ftr != -1:
                old_ftr_block = content[start_ftr:end_ftr + 9]
                content = content.replace(old_ftr_block, FULL_FOOTER)
...
def main():
    ...
    print(f"==================================================")
    print(f"[+] Successfully rebuilt header & footer in {count} HTML files!")
    print(f"==================================================")
